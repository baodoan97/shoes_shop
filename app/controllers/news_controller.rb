class NewsController < ApplicationController
  ...
  def index
    if params[:news_page] == nil || params[:news_page].to_i == 1
      @news = New.all.where(status: 'active').order("created_at DESC").paginate(page: params[:news_page], per_page: 3)
  ...
  def set_params
    @news =  params[:type] == " " ? New.all.where(status: 'active') : NewsTypeForShoe.find_by_name(params[:type]).new.where(status: 'active')
    @item = nil
    @news.each{ |item|
        if item[:title].gsub(%r{</?[^>]+?>|[^A-Za-z0-9 ]}, '') == params[:title] && item.status == 'active'
